/**
 * QualityJobsView - Quality Analysis Job Monitoring
 *
 * Displays quality analysis jobs (STT + Audio) with status, progress, and actions.
 * Similar to JobsView but for quality analysis jobs.
 *
...
import {
  ...
  useCancelQualityJob,
  useDeleteQualityJob,
  useClearQualityJobHistory,
  useResumeQualityJob,
} from '@hooks/useQualityQuery'
import { useSnackbar } from '@hooks/useSnackbar'
import { formatDistanceToNow } from 'date-fns'
import { de, enUS } from 'date-fns/locale'
import type { QualityJob } from '@types'
...
interface QualityJobCardProps {
  job: QualityJob
  onCancel: (job: QualityJob) => void
  onResume: (job: QualityJob) => void
  onDelete: (job: QualityJob) => void
  isCancelDisabled: boolean
  isResumeDisabled: boolean
  isDeleteDisabled: boolean
}

const QualityJobCard = memo(
  ({ job, onCancel, onResume, onDelete, isCancelDisabled, isResumeDisabled, isDeleteDisabled }: QualityJobCardProps) => {
    ...
    // Build job title: "Project / Chapter (Engines)"
    const jobTitle = useMemo(() => {
      ...
      if (job.sttEngine) engines.push(job.sttEngine)
      if (job.audioEngine) engines.push(job.audioEngine)
      const enginesStr = engines.length > 0 ? `(${engines.join(' + ')})` : ''

      // Combine
      if (location && enginesStr) {
        return `${location} ${enginesStr}`
      } else if (location) {
        return location
      } else if (enginesStr) {
        return enginesStr
      }
      return t('qualityJobs.analysis')
    }, [job.projectTitle, job.chapterTitle, job.sttEngine, job.audioEngine, t])
  ...
  (prevProps, nextProps) => {
    ...
    const propsChanged = prevProps.isCancelDisabled !== nextProps.isCancelDisabled || prevProps.isResumeDisabled !== nextProps.isResumeDisabled || prevProps.isDeleteDisabled !== nextProps.isDeleteDisabled
...
QualityJobCard.displayName = 'QualityJobCard'

interface QualityJobsViewProps {
  embedded?: boolean
}

const QualityJobsView = memo(({ embedded = false }: QualityJobsViewProps) => {
  const { t, i18n } = useTranslation()
  const dateLocale = i18n.language === 'de' ? de : enUS
  ...
  const { data: jobsData, isLoading, dataUpdatedAt } = useQualityJobs({ limit: 50 })
  const jobs = jobsData?.jobs ?? []

  const cancelJobMutation = useCancelQualityJob()
  const resumeJobMutation = useResumeQualityJob()
  const deleteJobMutation = useDeleteQualityJob()
  const clearHistoryMutation = useClearQualityJobHistory()
  ...
  const handleCancelJob = useCallback(
    async (job: QualityJob) => {
      try {
        await cancelJobMutation.mutateAsync(job.id)
        showSnackbar(t('qualityJobs.messages.cancelSuccess'), { severity: 'success' })
      } catch (err: unknown) {
        logger.error('[QualityJobsView] Failed to cancel job:', err)
        const errorMessage = translateBackendError(
          err instanceof Error ? err.message : t('qualityJobs.errors.cancelFailed'),
          t
        )
  ...
  const handleResumeJob = useCallback(
    async (job: QualityJob) => {
      try {
        ...
        showSnackbar(t('qualityJobs.messages.resumeSuccess'), { severity: 'success' })
      } catch (err: unknown) {
        logger.error('[QualityJobsView] Failed to resume job:', err)
        const errorMessage = translateBackendError(
          err instanceof Error ? err.message : t('qualityJobs.errors.resumeFailed'),
          t
        )
        showSnackbar(errorMessage, { severity: 'error' })
      }
  ...
  const handleDeleteJob = useCallback(
    async (job: QualityJob) => {
      try {
        ...
        showSnackbar(t('qualityJobs.messages.deleteSuccess'), { severity: 'success' })
      } catch (err: unknown) {
        logger.error('[QualityJobsView] Failed to delete job:', err)
        const errorMessage = translateBackendError(
          err instanceof Error ? err.message : t('qualityJobs.errors.deleteFailed'),
          t
        )
        showSnackbar(errorMessage, { severity: 'error' })
      }
  ...
  const handleClearHistory = useCallback(async () => {
    try {
      ...
      showSnackbar(t('qualityJobs.messages.clearHistorySuccess'), { severity: 'success' })
    } catch (err: unknown) {
      logger.error('[QualityJobsView] Failed to clear history:', err)
      const errorMessage = translateBackendError(
        err instanceof Error ? err.message : t('qualityJobs.errors.clearHistoryFailed'),
        t
      )
      showSnackbar(errorMessage, { severity: 'error' })
    }
  ...
  const formatTimestamp = useCallback(
    (timestamp: string | Date) => {
      try {
        const date = timestamp instanceof Date ? timestamp : new Date(timestamp)
        return formatDistanceToNow(date, { addSuffix: true, includeSeconds: true, locale: dateLocale })
      } catch {
        return String(timestamp)
      }
    },
  ...
  // CSS animation for rotating icon
  useEffect(() => {
    ...
    <>
      ...
      >
        {isLoading ? (
          ...
          <EmptyState
            icon={<HourglassEmpty />}
            message={t('qualityJobs.noJobs')}
            description={t('qualityJobs.noJobsDescription')}
          />
        ) : (
          <>
            {/* Active Jobs Section */}
                ...
                <List sx={{ p: 0 }}>
                  {activeJobs.map((job) => (
                    <QualityJobCard
                      ...
                      isCancelDisabled={cancelJobMutation.isPending}
            ...
            >
              {finishedJobs.length === 0 ? (
                <EmptyState
                  icon={<CheckCircle />}
                  message={t('qualityJobs.noFinishedJobs')}
                  sx={{ py: 3 }}
                />
              ) : (
                <List sx={{ p: 0 }}>
...
QualityJobsView.displayName = 'QualityJobsView'

export default QualityJobsView
