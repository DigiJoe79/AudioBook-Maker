...
async def execute_epub_import(
    ...
    # Filter and rename chapters if requested
    def should_import_chapter(chapter_title: str) -> bool:
        if not selected_chapters_list:
            return True
        return chapter_title in selected_chapters_list
    ...
    for chapter_data in parsed["chapters"]:
        ...
        if not should_import_chapter(original_title):
            continue
        ...
        for seg_data in chapter_data["segments"]:
            if seg_data["type"] == "text":
                ...
                segment_repo.create(
                    ...
                    char_count=seg_data.get("char_count"),
                    order_index=seg_data["order_index"],
                    pause_duration=seg_data.get("pause_duration"),
