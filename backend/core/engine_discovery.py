"""
Automatic Engine Discovery - Server Architecture

Scans backend/engines/ for engine server implementations.

Discovery Rules:
1. Scan engines/ for subdirectories (e.g., engines/xtts/)
2. Look for server.py (engine server script)
3. Look for engine.yaml (configuration)
4. Look for venv/ (virtual environment)
5. Parse metadata from engine.yaml
6. Return discovered engine metadata

Example:
    engines/xtts/
      - server.py        → Engine server
...
class EngineDiscovery:
    """
    Automatic discovery of TTS engine servers from engines/ directory

    Scans for engine server implementations (FastAPI servers running in
    separate VENVs) and parses their metadata from engine.yaml.
    """
    ...
    def discover_all(self) -> Dict[str, Dict[str, Any]]:
        ...
        logger.info(f"Starting engine discovery in: {self.engines_base_path}")
        ...
        logger.info(f"Engine discovery complete: {len(self.discovered_engines)} engines found")
    ...
    def _discover_engine(
        ...
        Args:
            engine_dir: Path to engine directory (e.g., engines/xtts/)
        ...
        if models_from_yaml:
            # Use explicit models from YAML (legacy)
    ...
    def _parse_models(
        ...
        for model_cfg in models_config:
            ...
            if not model_name or not model_path_str:
                logger.warning(f"Invalid model config: {model_cfg}")
                continue
            ...
            models.append({
                'tts_model_name': model_name,  # Use tts_ prefix to avoid Pydantic namespace conflict
                'path': str(model_path),
    ...
    def _auto_detect_models(self, engine_dir: Path) -> List[Dict[str, Any]]:
        ...
        if models:
            logger.info(f"Auto-detected {len(models)} model(s) in {engine_dir.name}: {[m['tts_model_name'] for m in models]}")
        else:
